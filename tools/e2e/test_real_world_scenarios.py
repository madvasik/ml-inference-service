...
class RealWorldScenarioTester:
    ...
    def scenario_2_multiple_users_concurrent(self) -> TestResult:
        ...
        try:
            ...
            for i in range(num_users):
                ...
                headers = {"Authorization": f"Bearer {token}"}
                response = requests.post(
                    f"{self.base_url}/api/v1/billing/payments",
                    headers=headers,
                    json={"amount": 200},
                    timeout=5
                )
    ...
    def scenario_10_security_checks(self) -> TestResult:
        ...
        try:
            ...
            # Пополняем балансы
            requests.post(f"{self.base_url}/api/v1/billing/payments", headers=headers1, json={"amount": 100}, timeout=5)
            requests.post(f"{self.base_url}/api/v1/billing/payments", headers=headers2, json={"amount": 100}, timeout=5)
    ...
    def scenario_11_history_and_audit(self) -> TestResult:
        ...
        try:
            ...
            if len(credit_transactions) < len(payment_amounts):
                return TestResult("scenario_11", False, f"Не все транзакции пополнения отображаются. Ожидалось: {len(payment_amounts)}, получено: {len(credit_transactions)}")
    ...
    def scenario_12_rate_limiting_real_world(self) -> TestResult:
        ...
        try:
            ...
            if rate_limited_count > 0:
                ...
                self.info(
                    "Rate limiting не достигнут при текущих дефолтных лимитах "
                    f"({successful_count} быстрых запросов подряд)"
                )
                return TestResult(
                    "scenario_12",
                    True,
                    (
                        "Rate limiting не достигнут при текущих настройках. "
                        f"Выполнено запросов: {successful_count}"
                    ),
                    {
                        "successful_requests": successful_count,
                        "rate_limited": 0
    ...
    def scenario_21_transaction_history_accuracy(self) -> TestResult:
        ...
        try:
            ...
            expected_amounts = set(payment_amounts)
